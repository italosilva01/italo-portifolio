...
const CardResumeProjects: React.FC<CardResumeProjectsProps> = ({ title, description, image, link }) => {
    return (
        <a href={link} target='_blank' rel="noopener noreferrer" className="w-fit">
            <Card.Root onClick={() => console.log(link)} className="max-w-[300px] max-h-[371px] h-[371px] flex flex-col gap-2 cursor-pointer">
                ...
                <Card.Image src={image} alt={title} width={300} height={160} className="rounded-2xl" />
            </Card.Root>
        </a>
    );
};
